"""
trapezoid.py: solves ODE with trapezoidal method
"""
...
def trapezoid(eval_f, p, u, x_start, t_start, t_stop, timestep, errf=1e-2, errDeltax=1e-2, relDeltax=1e-2, MaxIter=200):
    ...
    for n in range(int(np.ceil((t_stop-t_start)/timestep))):
        ...
        Xt = np.column_stack((Xt, X[:, n]))  # initial guess is the last point
        # xt stores intermediate solutions as columns
...
if __name__ == '__main__':
    ...
    delta_t = 5e-4
    times_rl, result_rl = trapezoid('evalf', p_rl, u_rl, x_rl, 0, 1, delta_t, 1e-2, 1e-2, 1e-2, 200)
    np.savetxt('results/TRAP_result_rl.txt', result_rl.T)

    times_ol, result_ol = trapezoid('evalf', p_ol, u_ol, x_ol, 0, 1, delta_t, 1e-2, 1e-2, 1e-2, 200)
    ...
    plt.title('Evening Steady-State (Trapezoidal)', fontsize=30)
    plt.show()

    # Uncomment to show dynamics plot
    # for idx, res in enumerate(result_rl):
    #     plt.plot(times_rl, res, label=f'Station {idx + 1}')
    # plt.title(r'Evening - Red Line (Trapezoidal, $\Delta t = 5 \cdot 10^{-4}$ h)')
    ...
    # plt.title(r'Evening - Orange Line (Trapezoidal, $\Delta t = 5 \cdot 10^{-4}$ h)')
    # plt.xlabel('Time (hours)')
    # plt.ylabel('Passengers at Station')
    # plt.show()
